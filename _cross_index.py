"""
Cruza faltantes com o smart_index ja construido (11097 ROMs, 1373 seriais).
Tambem busca por nome fuzzy nos 4863 nomes indexados.
"""
...
def main():
    print("=" * 70, flush=True)
    print("CROSS INDEX — cruzando faltantes com smart_index", flush=True)
    print("=" * 70, flush=True)

    # Carregar smart_index
    if not os.path.exists(INDEX_PATH):
        print("smart_index.json nao encontrado. Rode _smart_index.py primeiro.", flush=True)
        return
    ...
    for serial, name in sorted(missing.items()):
        ...
        # 2. Busca por serial com variacoes (sem hifen, com underscore)
        serial_variants = [
