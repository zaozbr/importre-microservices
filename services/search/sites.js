...
async function searchAll(serial, title) {
  ...
  const pureCacheNames = names.filter(n => pluginPriority(n) < 50 && isLocalCache(n) && isEnabled(n) && ['archive_org', 'archive_org_jp', 'archive_extra', 'myrient', 'homebrew', 'coolrom', 'romsdl', 'retrostic', 'vimm', 'psxdatacenter', 'archive_chd_jp', 'archive_redump_jp', 'archive_sony_play_station_japan_non_redump', 'archive_centuron_psx'].includes(n));
