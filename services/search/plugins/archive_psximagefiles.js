// Auto-gerado por _source_hunter.js para colecao archive.org: PSXImageFiles
...
const COLLECTION = 'PSXImageFiles';
...
module.exports = {
  ...
  async search(serial, title) {
    ...
    const results = scored.slice(0, 3).map(f =>
      buildSource('archive-psximagefiles', f.url, title, { score: f.score, size: f.size })
    );
    const magnet = getMagnetByCollection(COLLECTION);
    if (magnet) {
      results.push(buildSource('archive-psximagefiles-torrent', magnet, title, { score: 0.8, size: 0 }));
    }
    return results;
  }
};
