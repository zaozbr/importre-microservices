// Auto-gerado por _source_hunter.js para colecao archive.org: sony_playstation_part1
...
const COLLECTION = 'sony_playstation_part1';
...
module.exports = {
  name: 'archive-sony-playstation-part1',
  matchType: 'title',
  ...
  async search(serial, title) {
    ...
    const results = scored.slice(0, 3).map(f =>
      buildSource('archive-sony-playstation-part1', f.url, title, { score: f.score, size: f.size })
    );
    const magnet = getMagnetByCollection(COLLECTION);
    if (magnet) {
      results.push(buildSource('archive-sony-playstation-part1-torrent', magnet, title, { score: 0.8, size: 0 }));
    }
    return results;
  }
};
