// Auto-gerado por _source_hunter.js para colecao archive.org: sony-play-station-japan-non-redump
...
const COLLECTION = 'sony-play-station-japan-non-redump';
...
module.exports = {
  name: 'archive-sony-play-station-japan-non-redump',
  matchType: 'title',
  ...
  async search(serial, title) {
    ...
    const results = scored.slice(0, 3).map(f =>
      buildSource('archive-sony-play-station-japan-non-redump', f.url, title, { score: f.score, size: f.size })
    );
    const magnet = getMagnetByCollection(COLLECTION);
    if (magnet) {
      results.push(buildSource('archive-sony-play-station-japan-non-redump-torrent', magnet, title, { score: 0.8, size: 0 }));
    }
    return results;
  }
};
