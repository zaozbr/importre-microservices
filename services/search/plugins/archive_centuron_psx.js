// Auto-gerado por _source_hunter.js para colecao archive.org: Centuron-PSX
...
module.exports = {
  ...
  async search(serial, title) {
    ...
    const results = scored.slice(0, 3).map(f =>
      buildSource('archive-centuron-psx', f.url, title, { score: f.score, size: f.size })
    );
    // Adicionar source magnet (torrent) se disponivel
    const magnet = getMagnetByCollection(COLLECTION);
    if (magnet) {
      results.push(buildSource('archive-centuron-psx-torrent', magnet, title, { score: 0.8, size: 0 }));
    }
    return results;
  }
};
