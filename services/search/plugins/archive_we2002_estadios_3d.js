// Auto-gerado por _source_hunter.js para colecao archive.org: we2002-estadios-3d
const axios = require('axios');
const { normalize, titleScore, buildSource } = require('./_base');

const HEADERS = { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' };
const COLLECTION = 'we2002-estadios-3d';
...
module.exports = {
  name: 'archive-we2002-estadios-3d',
  matchType: 'title',
  ...
  async search(serial, title) {
    ...
    return scored.slice(0, 3).map(f =>
      buildSource('archive-we2002-estadios-3d', f.url, title, { score: f.score, size: f.size })
    );
  }
};
