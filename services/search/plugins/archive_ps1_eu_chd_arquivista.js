// Auto-gerado por _source_hunter.js para colecao archive.org: PS1_EU_CHD_Arquivista
...
const COLLECTION = 'PS1_EU_CHD_Arquivista';
...
module.exports = {
  name: 'archive-ps1-eu-chd-arquivista',
  matchType: 'title',
  ...
  async search(serial, title) {
    ...
    const results = scored.slice(0, 3).map(f =>
      buildSource('archive-ps1-eu-chd-arquivista', f.url, title, { score: f.score, size: f.size })
    );
    const magnet = getMagnetByCollection(COLLECTION);
    if (magnet) {
      results.push(buildSource('archive-ps1-eu-chd-arquivista-torrent', magnet, title, { score: 0.8, size: 0 }));
    }
    return results;
  }
};
