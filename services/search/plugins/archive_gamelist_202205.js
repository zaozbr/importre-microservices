// Auto-gerado por _source_hunter.js para colecao archive.org: gamelist_202205
...
const COLLECTION = 'gamelist_202205';
...
module.exports = {
  ...
  async search(serial, title) {
    ...
    const results = scored.slice(0, 3).map(f =>
      buildSource('archive-gamelist-202205', f.url, title, { score: f.score, size: f.size })
    );
    const magnet = getMagnetByCollection(COLLECTION);
    if (magnet) {
      results.push(buildSource('archive-gamelist-202205-torrent', magnet, title, { score: 0.8, size: 0 }));
    }
    return results;
  }
};
