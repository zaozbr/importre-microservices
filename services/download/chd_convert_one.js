  ...
  while (i < data.length) {
    ...
    if (type === 0) {
      ...
      while (num > 0) {
        let b = num;
        if (b > 2352) b = 2352;
        const raw = data.subarray(i, i + b);
        checkedc = edc_partial_computeblock(checkedc, raw, b);
      ...
      while (num > 0) {
        ...
        switch (type) {
          case 1: // Mode 1
            sector[0x0F] = 0x01;
            sector.set(data.subarray(i, i + 0x003), 0x00C); i += 0x003;
            sector.set(data.subarray(i, i + 0x800), 0x010); i += 0x800;
          ...
          case 2: // Mode 2 form 1
            sector[0x0F] = 0x02;
            sector.set(data.subarray(i, i + 0x804), 0x014); i += 0x804;
            sector[0x10] = sector[0x14];
            sector[0x11] = sector[0x15];
            sector[0x12] = sector[0x16];
            sector[0x13] = sector[0x17];
            eccedc_generate(sector, 2);
            checkedc = edc_partial_computeblock(checkedc, sector.subarray(0x10), 2336);
            chunks.push(Buffer.from(sector.subarray(0x10, 0x10 + 2336)));
            break;
          case 3: // Mode 2 form 2
            sector[0x0F] = 0x02;
            sector.set(data.subarray(i, i + 0x918), 0x014); i += 0x918;
            sector[0x10] = sector[0x14];
            sector[0x11] = sector[0x15];
            sector[0x12] = sector[0x16];
            sector[0x13] = sector[0x17];
